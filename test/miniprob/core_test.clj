...
(deftest flip-args-exist-test
  (testing "when eval is provided with a trace and flip is provided with a call-site tag, flip
      returns the expected value from the trace"
    (is (= (eval '(flip :tag) {:tag 0}) 0))
    (is (= (eval '(+ (flip :a) (flip :b)) {:a 2 :b 3}) 5))))

(deftest flip-args-do-not-exist-test
  (testing "when eval is provided with a trace and flip is provided with a call-site tag, flip
      returns either 0 or 1 if the tag does not exist in the trace"
    (let [results (repeatedly 100 #(eval '(flip :tag) {}))]
      (is (zero? (count (filter #(and (not (zero? %)) (not= 1 %)) results)))))))

(deftest flip-args-no-args-test
  (testing "when eval is provided with a trace, and some but not all flips are provided with a
      call-site tag, flip returns expected trace values when indicated and either 0 or 1 otherwise"
    (let [results (repeatedly 100 #(eval '(+ (flip) (flip :a) (flip :b)) {:a 0 :b 1}))]
